...
def cuentos(image):
    ...
    while True:
        ...
        if k == 49:
            image = cv2.imread('Img/perro.png', cv2.IMREAD_UNCHANGED)
        if k == 50:
            image = cv2.imread('Img/tigre.png', cv2.IMREAD_UNCHANGED)
        if k == 51:
            image = cv2.imread('Img/sandia.png', cv2.IMREAD_UNCHANGED)
        if k == 52:
            image = cv2.imread('Img/cereza.png', cv2.IMREAD_UNCHANGED)
        if k == 53:
            image = cv2.imread('Img/policia.png', cv2.IMREAD_UNCHANGED)
        if k == 54:
            image = cv2.imread('Img/enfermera.png', cv2.IMREAD_UNCHANGED)
        if k == 55:
            image = cv2.imread('Img/frio.png', cv2.IMREAD_UNCHANGED)
        if k == 56:
            image = cv2.imread('Img/calor.png', cv2.IMREAD_UNCHANGED)
        ret, frame = cap.read()
        if ret == False: break
